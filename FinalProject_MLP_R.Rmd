...
```{r}
#Predicting Singapores's rank rising if they improve their Quality of Life and cultural influence

# Get Singapore
singapore_row <- data_scaled[rankings_clean$Brand == "Singapore", ]
...
singapore_row[important_attributes] <- pmin(singapore_row[important_attributes] * 0.8, 1)

# Predict the improved rank
singapore_pred_scaled <- pmax(predict(nn_model, singapore_row), 0)

# Rescale to real rank
singapore_pred_real <- rescale_to_real(singapore_pred_scaled, min_y, max_y)

# Compare original vs improved prediction
original_rank <- rescale_to_real(data_scaled[rankings_clean$Brand == "Singapore", "OVERALL_R"], min_y, max_y)
